    ...
    $.widget("wy.wyTabview",{
        ...
        _createTabTrigger:function(tab){
            ...
            if(tab.hasClass(classDef.closable)){
                ...
                tab.children("." + this.parts.closeTab).click(function(e){
                    if (_self.options.closeTab) {
                        if (!_self.options.closeTab(e)) {
                            return;
                        }
                    }
        ...
        _appendNewTab:function(tab){
            ...
            if(tab.contentType=="iframe"){
                $content.html("<iframe src='"+ tab.content +"' frameborder='0' id='f_"+ tab.id +"' scrolling='auto' marginheight='0' marginwidth='0'></iframe>");
            }
        ...
        add:function(tab,ignoreSame){
            ...
            //判断当前标题的页签是否存在
            this.header.find("." + this.parts.tab).each(function(){
                var hasTabs = $.trim(tab.title) === $.trim($(this).children("div").text());
                hasTabs = $.trim(tab.key) == $.trim($(this).data("id"));
                if(hasTabs){
                    //TODO:1\实现是否刷新已有tab
